...
#
# Time Complexity:
# - Building the heap takes O(n) time since each call to init_heap operates in O(log n), and there are n/2 nodes to call init_heap on.
# - Extracting elements: Each extraction takes O(log n) because the init_heap function runs in O(log n), and we do this n-1 times (since the last element is already sorted).
...
def init_heap(arr, n, i):
    ...
    if largest != i:
        ...
        init_heap(arr, n, largest)  # Recursively "heapify" the affected subtree
...
def heapSort(arr):
    ...
    for i in range(n - 1, 0, -1):
        ...
        init_heap(arr, i, 0)  # Restore heap property on the reduced heap
